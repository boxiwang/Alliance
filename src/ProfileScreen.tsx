...
export default function ProfileScreen({
  ...
  return <section className={`profile-screen ${account.reducedMotion ? "profile-motion-stilled" : ""}`}>
    ...
    {section === "protocols" && <div className="profile-system-grid">
      ...
      <article className="profile-card"><header><small>RENDER PROTOCOL</small><span>THIS DEVICE</span></header><div className="profile-fields"><label><span>STARMAP FIDELITY</span><select value={account.graphicsTier} onChange={(event) => commitAccount({ ...account, graphicsTier: event.target.value as GraphicsTier })}>{(["auto", "ultra", "high", "medium", "low"] as GraphicsTier[]).map((tier) => <option value={tier} key={tier}>{GRAPHICS_TIER_LABEL[tier]}{tier === "auto" ? ` → ${GRAPHICS_TIER_LABEL[detectAutoTier()].toUpperCase()}` : ""}</option>)}</select></label><p className="profile-field-hint">{GRAPHICS_TIER_HINT[account.graphicsTier]}{account.reducedMotion ? " · Reduced Motion holds every signature still." : ""}</p></div></article>
      <article className="profile-card"><header><small>SHIP SYSTEMS</small><span>SYNCED</span></header><div className="profile-switch-list"><SwitchRow title="Background music" detail="CELESTIAL DRIFT · CONTINUOUS" checked={account.musicEnabled} onChange={(value) => commitAccount({ ...account, musicEnabled: value })} /><div className={`profile-volume-control ${account.musicEnabled ? "live" : "muted"}`}><span><b>Music volume</b><small>BACKGROUND SCORE · {Math.round(account.musicVolume * 100)}%</small></span><input aria-label="Music volume" type="range" min="0" max="100" step="1" value={Math.round(account.musicVolume * 100)} onChange={(event) => commitAccount({ ...account, musicVolume: Number(event.target.value) / 100 }, false)} onPointerUp={() => flash("MUSIC LEVEL SET")} /></div><SwitchRow title="Sound effects" detail="CLICKS · FLEETS · ALERTS · COMMS" checked={account.soundEnabled} onChange={(value) => commitAccount({ ...account, soundEnabled: value })} /><div className={`profile-volume-control ${account.soundEnabled ? "live" : "muted"}`}><span><b>Sound effects volume</b><small>CLICKS · SELECTIONS · COMMS · {Math.round(account.sfxVolume * 100)}%</small></span><input aria-label="Sound effects volume" type="range" min="0" max="100" step="1" value={Math.round(account.sfxVolume * 100)} onChange={(event) => commitAccount({ ...account, sfxVolume: Number(event.target.value) / 100 }, false)} onPointerUp={() => flash("EFFECTS LEVEL SET")} /></div><SwitchRow title="Still the cosmos" detail="REDUCED MOTION" checked={account.reducedMotion} onChange={(value) => commitAccount({ ...account, reducedMotion: value })} /><SwitchRow title="Translate unknown tongues" detail="AUTO-TRANSLATE COMMS" checked={account.autoTranslateComms} onChange={(value) => commitAccount({ ...account, autoTranslateComms: value })} /><SwitchRow title="Raise battle alarms" detail="ATTACKS · RALLIES · ORDERS" checked={account.criticalNotifications} onChange={(value) => commitAccount({ ...account, criticalNotifications: value })} /></div></article>
